	...
	m_orientation(orientation)
{
	...
	connect(axisTime, &QDateTimeAxis::rangeChanged, this, &CrossHair::rangeChanged);
...
void CrossHair::updateTextFromValue()
{
	if (m_orientation == Qt::Horizontal) {
		...
		auto time = quintToTime(m_value.x());
		//m_movingLabel->setText(time.toString("dd"));
		m_movingLabel->setText(time.toString("HH:mm"));
	}
}
...
{
	if (m_orientation != Qt::Vertical || !m_chart->unitSecSpan())
		return;
	auto currentTime = quintToTime(m_value.x());
	qreal span = m_chart->startTime().secsTo(currentTime);
	int numSpans = qRound(span / (qreal)m_chart->unitSecSpan());
	auto roundTime = m_chart->startTime().addSecs(numSpans * m_chart->unitSecSpan());
	m_value.setX(timeToQint(roundTime));
